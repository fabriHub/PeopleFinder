...
public class DAOGruppo implements IDAOGruppo {
	...
	public List<String[]> findAllGestioneGruppi(Long id) throws DAOException {
		...
		try {
			statement = connection.prepareStatement("SELECT GRUPPO.ID, ATTIVITA.NOME, DATA_EVENTO, DESCRIZIONE FROM GRUPPO JOIN UTENTE ON UTENTE.ID = GRUPPO.ID_UTENTE JOIN ATTIVITA ON ATTIVITA.ID = GRUPPO.ID_ATTIVITA WHERE UTENTE.ID = ? AND COMPLETO = 0 ORDER BY DATA_EVENTO ASC");
	...
	public List<String[]> panoramicaGruppi() throws DAOException {
		...
		try {
			statement = connection.prepareStatement("SELECT GRUPPO.ID, UTENTE.NICKNAME, ATTIVITA.NOME, DATA_EVENTO, DESCRIZIONE, COMPLETO FROM GRUPPO JOIN UTENTE ON UTENTE.ID = GRUPPO.ID_UTENTE JOIN ATTIVITA ON ATTIVITA.ID = GRUPPO.ID_ATTIVITA ORDER BY COMPLETO DESC");
			resultSet = statement.executeQuery();
			while (resultSet.next()) {
				String[] gruppo = new String[6];
				gruppo[0] = String.valueOf(resultSet.getLong("ID"));
